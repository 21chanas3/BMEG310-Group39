...
pacman::p_load(BiocManager, tidyverse, reshape2, pheatmap, survminer, corrplot)
...
```{r}
gene.corr.matrix <- patient.mutation.data |>
  mutate(present = 1) |>
  pivot_wider(names_from = Hugo_Symbol, values_from = present, values_fill = 0) |>
  select(-PATIENT_ID, -DSS_STATUS) |>
  cor()


corrplot(gene.corr.matrix, type = "upper", diag = F)
```
```{r}
genes.by.corr <- gene.corr.matrix[,"DSS_MONTHS"] |>
    tidy() |>
    filter(names != "DSS_MONTHS") |>
    arrange(desc(x))
