...
mutation_filtered <- subset(mutation_intersect,IMPACT =="HIGH")
```
...
patient.mutation.data <- mutation_data |> 
  ...
  merge(patient_data[, c("PATIENT_ID", "OS_MONTHS", "DSS_MONTHS")], by = "PATIENT_ID") |>
  distinct()
```
...
## Expression Analysis
```{r}
countData <- count_matrix_intersect #just to keep the same variable name in the tutorial
