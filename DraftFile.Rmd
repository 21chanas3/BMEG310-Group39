...
## Filter for high impact and Regenerate MAF Summary
```{r}
...
ggsurvplot(fit,data=survdata,pval=T,title="PFS Rate")
```
...
pheatmap(sampleMatrix , cluster_rows=FALSE, show_rownames=TRUE,
         cluster_cols=TRUE, annotation_col=annot_col,clustering_method = "ward.D", show_colnames =F ,labels_row = rownames(resSig[tp20udgenes,]))
```
...
```
```{r}
pathview(gene.data=foldchanges, pathway.id="hsa04110", species="hsa")
```
